...
if [ -f "${SITE_DIR}/daily_psurf_values.txt" ]; then
    echo ""daily_psurf_values.txt" exists. Deleting..."
    rm ${SITE_DIR}/daily_psurf_values.txt
    echo "File deleted."
else
    echo "File "daily_psurf_values.txt" does not exist. It will be created."
fi

PSURF_FILE=${SITE_DIR}/daily_psurf_values.txt
...
for file in "${files[@]}"; do
    ...
    ncap2 -O -s "lat={${SITE_LAT}}" 7.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ...
    for file in {8..11}.nc; do
        ...
        ncks --trd -H -v lev ${SITE}_${DAY}_${MONTH}_${YEAR}.nc | awk 'BEGIN {FS="="} /lev/ {printf("%8.1f\n", $2)}' > temp_file.txt
        ...
        if [ "$(echo "$h_below == 0" | bc -l)" -eq 1 ]; then
            H_below+=(0)
            SLP=$(ncks -s '%f\n' -H -C -v SLP ${SITE}_${DAY}_${MONTH}_${YEAR}.nc | head -n 1)
    ...
    # Compute means
    ncap2 -O -S $SCRIPTS_DIR/../T_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../RH_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../QL_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../QI_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../O3_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../U_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ncap2 -O -S $SCRIPTS_DIR/../V_avgs.nco ${SITE}_${DAY}_${MONTH}_${YEAR}.nc ${SITE}_${DAY}_${MONTH}_${YEAR}.nc
    ...
    # format is CDL.
    ncks --trd -H -v lev ${SITE}_${DAY}_${MONTH}_${YEAR}.nc |
    ...
    echo "#  P[mb]  T_avg[K] H2O_vpr_avg[1] lqd_H2O_avg[ppm] ice_H2O_avg[ppm] O3_avg[ppm] v_wind_avg[m/s]" \
        > ${OUTDIR_PROFILES}/${SITE}_${DAY}_${MONTH}_${YEAR}_MERRA_means.txt
    ...
    echo "PSURF_${DAY}_${MONTH}_${YEAR}=$PS" >> "$PSURF_FILE"

    # Remove rows with unphysical temperature values
    $SCRIPTS_DIR/.././filter_rows.sh ${OUTDIR_PROFILES}/${SITE}_${DAY}_${MONTH}_${YEAR}_MERRA_means.txt

    awk -f $SCRIPTS_DIR/../extrapolate_to_surface.awk Ptrunc=$PTRUNC Ps=$PS ${OUTDIR_PROFILES}/${SITE}_${DAY}_${MONTH}_${YEAR}_MERRA_means.txt > ${OUTDIR_PROFILES}/${SITE}_${DAY}_${MONTH}_${YEAR}_MERRA_means_ex.txt
