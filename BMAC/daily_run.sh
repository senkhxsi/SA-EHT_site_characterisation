...
export DATADIR=${SITE_DIR}/march_24
export SCRIPTS_DIR=~/projects/ngeht_site_characterisation/scripts/daily
...
export OUTDIR_PROFILES=${SITE_DIR}/daily_profile_stats
if [ ! -d $OUTDIR_PROFILES ]; then
    mkdir $OUTDIR_PROFILES
fi
echo computing daily statistics ...
$SCRIPTS_DIR/./daily_profile_stats.sh

# Generate am model files and compute spectra.
export OUTDIR_AM=${SITE_DIR}/daily_am_models
if [ ! -d $OUTDIR_AM ]; then
    mkdir $OUTDIR_AM
fi
echo generating am model files ...
$SCRIPTS_DIR/./daily_make_am_models.sh
echo computing am models ...
$SCRIPTS_DIR/./daily_run_am_models.sh
...
echo done. Outputs written to $OUTDIR_PROFILES, $OUTDIR_AM.
